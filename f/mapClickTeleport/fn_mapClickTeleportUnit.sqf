...
["MapClickTeleport",["Left-Click to select location."]] call BIS_fnc_showNotification;
openMap [true, false];
...
if (f_var_mapClickTeleport_Height > 0) then {
	[player] spawn {
		...
		if (a2k_autoDeploy) then {
			waitUntil {sleep 0.1; getPosATL player select 2 < f_var_mapClickTeleport_Height};
			sleep 1;
...
if (f_var_mapClickTeleport_Uses == 0 || f_var_mapClickTeleport_Used < f_var_mapClickTeleport_Uses) then {
	//f_mapClickTeleportAction = player addaction [deployAction,{[] spawn f_fnc_mapClickTeleportUnit},"", 0, false,true,"","_this == player"];
