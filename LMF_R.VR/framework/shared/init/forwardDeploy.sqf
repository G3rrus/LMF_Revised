...
private _statement_register = {
	...
	private _mapClickEH_deploy = addMissionEventHandler ["MapSingleClick",{
		...
		if (markerText _mrkName isEqualTo "") exitWith {
			...
			lmf_deploymarker setMarkerColor var_markerSide;
	...
	[{
		...
		if !(isNil "lmf_deploymarker") then {
			...
			if (player isEqualTo leader _grp) then {
				...
				_lmf_deploymarkerR setMarkerColor var_markerSide;
				...
				titleText ["<t font='PuristaBold' shadow='2' color='#FFBA26' size='1.6'>Group registered.</t>","PLAIN",0.3,false,true];
			} else {
				deleteMarker lmf_deploymarker;
...
private _statement_register_vic = {
	...
	if (_variable isEqualType true) then {
		...
		private _text = format ["<t font='PuristaBold' shadow='2' color='#FFBA26' size='1.6'>This vehicle is already owned by %1.</t>",groupId _variable];
